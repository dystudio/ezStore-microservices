...
{
    public class BaseModelRepository<TModel> : IWritableDataAccess, IDataAccessWriteRepository<TModel>, IDataAccessReadOnlyRepository<TModel> where TModel : class
    {
            ...
            int page = 1, int pageSize = 20)
        {
            ...
            if (!string.IsNullOrEmpty(orderBy))
            {
                var orderQuery = _dbSet.ApplyOrder(orderBy, methodName);
                IOrderedQueryable<TModel> orderByFunc(IQueryable<TModel> i) => orderQuery;
                return await GetPagedAsync(filter, orderByFunc, includeProperties, isIncludedIsDeleted);
            }
            return await GetPagedAsync(filter, null, includeProperties, isIncludedIsDeleted, page, pageSize);
        }
        ...
        public async Task<TModel> FirstOrDefaultAsync(Expression<Func<TModel, bool>> filter = null,
            Func<IQueryable<TModel>, IOrderedQueryable<TModel>> orderBy = null,
             string includeProperties = "", bool isIncludedIsDeleted = true)
        {
            return await Get(filter, orderBy, includeProperties, isIncludedIsDeleted).FirstOrDefaultAsync();
        }
