...
{
    public class BaseModelRepository<TModel> : IDataAccessWriteRepository<TModel>, IDataAccessReadOnlyRepository<TModel> where TModel : ModelEntity<ObjectId>
    {
        ...
        public async Task<PagedResult<TModel>> GetPagedAsync(Expression<Func<TModel, bool>> filter,
            Func<IQueryable<TModel>, IOrderedQueryable<TModel>> orderBy,
            string includeProperties = "", bool isIncludedIsDeleted = true,
            int page = 1, int pageSize = 20)
        {
        ...
        public async Task<PagedResult<TModel>> GetPagedAsync(Expression<Func<TModel, bool>> filter = null,
            string orderBy = null, bool orderAsc = true,
            string includeProperties = "", bool isIncludedIsDeleted = true,
            int page = 1, int pageSize = 20)
        {
            ...
            if (!string.IsNullOrEmpty(orderBy))
            {
                var orderQuery = query.ApplyOrder(orderBy, methodName);
                IOrderedQueryable<TModel> orderByFunc(IQueryable<TModel> i) => orderQuery;
                return await Task.Run(() => { return GetPaged(filter, orderByFunc, includeProperties, isIncludedIsDeleted); });
            }
            return await Task.Run(() => { return GetPaged(filter, null, includeProperties, isIncludedIsDeleted, page, pageSize); });
        }
        ...
        public async Task<TModel> FirstOrDefaultAsync(Expression<Func<TModel, bool>> filter = null, Func<IQueryable<TModel>, IOrderedQueryable<TModel>> orderBy = null,
             string includeProperties = "", bool isIncludedIsDeleted = true)
        {
            return await Task.Run(() => { return Get(filter, orderBy, includeProperties, isIncludedIsDeleted).FirstOrDefault(); });
        }
